...
public class TitanEventWriter {
	private static String THIS_CLASS_NAME = "TitanEventWriter";
	...
	private static int commitCount = 3000;
	...
	public static void main(String[] args) throws Exception {
		if(args.length <  3) {
			LOGGER.severe("Argumests needed : <frontHost> <frontPort> <dbHost> <commit count>");
			System.exit(1);
		} else {
				frontHost = args[0];
				frontPort = args[1]; 
				dbHost = args[2];
				if(args.length == 3) {
					LOGGER.info("Default commit count (" + commitCount + ") used.");
				} else {
					try {
						commitCount = Integer.parseInt(args[3]);
